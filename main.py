...
root = Tk()
root.title("Medical Intelligence")
...
def login():
    ...
    if counterlogin == 1:
        ...
        def Blsearch(ID):
            global Blsearch
            connection = sqlite3.connect("SHIVANI.db")
        ...
        def savebill():
            ...
            if BlIdentity.get() == "" or BlName.get() == "" or GenderBl.get() == "" or int(
                    BlAge.get()) == "" or Blcheckin.get() == "" or BlDisease.get() == "" or int(
                    BlPhone.get()) == "" or BlDoctorname.get() == "" or Blcheckout.get() == "" or int(
                    BlDoctorcharges.get()) == "" or int(BlLabtest.get()) == "" or int(
                    TreatmentchargesBl.get()) == "" or int(BlProcedurecharges.get()) == "" or int(
                    BlOthercharges.get()) == "" or int(BlMiscellaneouscharges.get()) == "" or int(
                    BlRoomcharges.get()) == "":
                result = tkMessageBox.showwarning('', 'Please Complete The Required Field', icon="warning")
            else:
                connection = sqlite3.connect("SHIVANI.db")
                cursor = connection.cursor()
                query = "insert into bl values('{}','{}','{}','{}','{}','{}','{}','{}','{}','{}','{}','{}','{}','{}','{}','{}','{}','{}')".format(
                    BlIdentity.get(), BlName.get(), GenderBl.get(), BlAge.get(), Blcheckin.get(), BlDisease.get(),
                    BlPhone.get(), BlAddress.get(), BlDoctorname.get(), Blcheckout.get(), BlDoctorcharges.get(),
                    BlLabtest.get(), TreatmentchargesBl.get(), BlProcedurecharges.get(), BlOthercharges.get(),
                    BlMiscellaneouscharges.get(), BlRoomcharges.get(), BlTotal.get())
                cursor.execute(query)
                connection.commit()
        ...
        def SubmitAppointment():
            ...
            cursor = connection.cursor()
            cursor.execute(
                "INSERT INTO ap (patientnoappointment,patientnameappointment,genderappointment,ageappointment,phonenoappointment,addressappointment,appointmenttime) VALUES(?, ?, ?, ?, ?, ?,?)",
                (str(PATIENTNOAPPOINTMENT.get()), str(PATIENTNAMEAPPOINTMENT.get()), str(GENDERAPPOINTMENT.get()),
                 str(AGEAPPOINTMENT.get()), str(PHONENOAPPOINTMENT.get()), str(ADDRESSAPPOINTMENT.get()),
                 str(APPOINTMENTTIME.get())))
            connection.commit()
        ...
        def updateappointment():
            ...
            query = "update ap set patientnameappointment='{}', genderappointment='{}',ageappointment='{}',phonenoappointment='{}',addressappointment='{}',appointmenttime='{}' where patientnoappointment='{}'".format(
                PATIENTNAMEAPPOINTMENT.get(), GENDERAPPOINTMENT.get(), AGEAPPOINTMENT.get(), PHONENOAPPOINTMENT.get(),
                ADDRESSAPPOINTMENT.get(), APPOINTMENTTIME.get(), PATIENTNOAPPOINTMENT.get())
            cursor.execute(query)
            connection.commit()
        ...
        def Billsearch(ID):
            connection = sqlite3.connect("SHIVANI.db")
            cursor = connection.cursor()
            query = "select * from pr where PrPatientid like '{0}' or PrName like '{0}' or PrPhone like '{0}'".format(
                ID)
            cursor.execute(query)
            for row in cursor:
        ...
        def appointment():
            ...
            if counterappointment == 1:
                frameappointment = Frame(root, bg="grey")
                ...
                labelappointment = Label(frameappointmentleft, text="Appointment", bg="grey", fg="white",
                                         font=("calibri", 30, "bold"))
                labelappointment.pack(anchor="nw")

                frameappointmentlef = Frame(frameappointment, bg="grey", height=500, width=500)
                frameappointmentlef.pack(side=TOP, pady=25)

                patintnoappointment = Label(frameappointmentlef, text="Appointment No:", bg="grey", fg="white",
                                            font=("calibri", 20, "bold"))
                patintnoappointment.grid(row=0, column=0)

                PATIENTNOAPPOINTMENT = StringVar()
                entrypatientnoappointment = Entry(frameappointmentlef, textvariable=PATIENTNOAPPOINTMENT,
                                                  font=("calibri", 20, "bold"))
                entrypatientnoappointment.grid(row=0, column=1)

                patientnameappointment = Label(frameappointmentlef, text="Patient Name:    ", bg="grey",fg="white",
                                               font=("calibri", 20, "bold"))
                patientnameappointment.grid(row=1, column=0, pady=20)

                PATIENTNAMEAPPOINTMENT = StringVar()
                entrymenucode = Entry(frameappointmentlef, textvariable=PATIENTNAMEAPPOINTMENT,
                                      font=("calibri", 20, "bold"))
                ...
                labelGender = Label(frameappointmentlef, text="Gender:               ", bg="grey",fg="white",
                                    font=("calibri", 20, "bold"))
                labelGender.grid(row=2, column=0)

                i = IntVar()
                r1 = Radiobutton(frameappointmentlef, text="male", value='Male', variable=GENDERAPPOINTMENT,fg="white",
                                 bg="grey", font=("calibri", 20, "bold"))
                r2 = Radiobutton(frameappointmentlef, text="female", value='Female', variable=GENDERAPPOINTMENT,fg="white",
                                 bg="grey", font=("calibri", 20, "bold"))
                r1.grid(row=2, column=1, sticky="w")
                r2.grid(row=2, column=1, sticky="e", padx=0)

                labelItemName = Label(frameappointmentlef, text="Age:                 ", bg="grey",fg="white",
                                      font=("calibri", 20, "bold"))
                ...
                labelQuantity = Label(frameappointmentlef, text="Phone No:         ", bg="grey",fg="white",
                                      font=("calibri", 20, "bold"))
                labelQuantity.grid(row=4, column=0, pady=20)

                PHONENOAPPOINTMENT = StringVar()
                entryquantity = Entry(frameappointmentlef, textvariable=PHONENOAPPOINTMENT,
                                      font=("calibri", 20, "bold"))
                entryquantity.grid(row=4, column=1, pady=20)

                labelItemName = Label(frameappointmentlef, text="Address:         ", bg="grey",fg="white",
                                      font=("calibri", 20, "bold"))
                labelItemName.grid(row=5, column=0, pady=20)

                ADDRESSAPPOINTMENT = StringVar()
                entryitemname = Entry(frameappointmentlef, textvariable=ADDRESSAPPOINTMENT,
                                      font=("calibri", 20, "bold"))
                entryitemname.grid(row=5, column=1, pady=20)

                labelGender = Label(frameappointmentlef, text="Time/Date:        ", bg="grey",fg="white",
                                    font=("calibri", 20, "bold"))
                ...
                buttonADD = Button(frameappointmentlef, text="Add", command=SubmitAppointment, bg="black", fg="black",
                                   width="12", font=("calibri", 20))
                buttonADD.grid(row=7, column=0, sticky="e")

                buttonUpdate = Button(frameappointmentlef, text="Update", command=updateappointment, bg="black",
                                      fg="black", width="12", font=("calibri", 20))
                ...
                Edititem = Label(frameappointmentrecord, text="Appointment List:-", fg="black",
                                 font=("calibri", 30, "bold"))
                ...
                search_by = StringVar()
                combo_search = ttk.Combobox(frameButton, textvariable=search_by, width=7,
                ...
                txt_search = Entry(frameButton, textvariable=search_txt, font=("times new roman", 10, "bold"), bd=5,
                                   relief=GROOVE, width=10)
                txt_search.grid(row=0, column=2, padx=5, pady=10, sticky="w")

                searchbtn = Button(frameButton, text="Search", command=search_data, width=10).grid(row=0, column=3,
                                                                                                   pady=10)
                showallbtn = Button(frameButton, text="Show All", width=10, command=fetchappointment_data).grid(row=0,
                                                                                                                column=4,
                                                                                                                pady=10)
                showallbtn = Button(frameButton, text="Delete", width=10, command=DeleteData).grid(row=0, column=5,
                                                                                                   pady=10)
                ...
                appointment_table = ttk.Treeview(frameappointmentright, columns=(
                "Patientnoappointment", "Patientnameappointment", "Genderappointment", "Ageappointment",
                "Phonenoappointment", "Addressappointment", "Appointmenttime"), xscrollcommand=scroll1.set,
        ...
        def billrecord():
            ...
            if counterbillrecord == 1:
                framebillrecord = Frame(root, relief="solid", borderwidth=2, bg="grey")
                framebillrecord.pack(side="right", fill=BOTH, expand=True)

                frameybillr = Frame(framebillrecord, bg="white", relief="solid", borderwidth=2)
                frameybillr.pack(side=TOP, fill=X)

                labelbill = Label(frameybillr, text="Bill Record", bg="white", fg="black",
                                  font=("calibri", 20, "bold"))
                labelbill.pack()

                frameq = Frame(framebillrecord, width=100, relief="solid", borderwidth=2, bg="grey")
                frameq.pack(side="left", fill=Y)

                labelinputid = Label(frameq, text="Input Patient id", fg="white", bg="grey",
                                     font=("calibri", 20, "bold"))
                ...
                Searchbutton = Button(frameq, text="Search", command=lambda: Blsearch(searchbillrecord.get()),
                                      bg="yellow", font=("calibri", 15, "bold"))
                Searchbutton.pack(side=TOP, pady=8)

                framezbill = Frame(framebillrecord, bg="yellow", relief="solid", borderwidth=2)
                framezbill.pack(side=TOP, fill=X)

                frametbill = Frame(framebillrecord, height="200", bg="grey")
                frametbill.pack(side=TOP, fill=X)

                framebrleft = Frame(frametbill, height="200", width="685", bg="grey")
                framebrleft.pack(side=LEFT)

                labelPatient_id = Label(framebrleft, text="Identity:", bg="grey",fg="white", font=("calibri", 16, "bold"))
                ...
                labelName = Label(framebrleft, text="Name:", bg="grey",fg="white", font=("calibri", 16, "bold"))
                ...
                labelGender = Label(framebrleft, text="Gender:  ", bg="grey", fg="white",font=("calibri", 16, "bold"))
                labelGender.grid(row=2, column=0)

                i = IntVar()
                r1 = Radiobutton(framebrleft, text="male", value='Male', variable=GenderBlrecord, bg="grey",fg="white",
                                 font=("calibri", 16, "bold"))
                r2 = Radiobutton(framebrleft, text="female", value='Female', variable=GenderBlrecord, bg="grey",fg="white",
                                 font=("calibri", 16, "bold"))
                r1.grid(row=2, column=1, sticky="w")
                r2.grid(row=2, column=1, sticky="e", padx=0)
                GenderBlrecord.set(3)

                labelAge = Label(framebrleft, text="Age:", bg="grey",fg="white", font=("calibri", 16, "bold"))
                ...
                labelCheckIn = Label(framebrleft, text="Check In:", bg="grey",fg="white", font=("calibri", 16, "bold"))
                ...
                framebrights = Frame(frametbill, height="200", bg="grey", width="685")
                framebrights.pack(side=RIGHT)

                labelAge = Label(framebrights, text="Disease:", bg="grey",fg="white", font=("calibri", 16, "bold"))
                ...
                labelPhone = Label(framebrights, text="Phone:", bg="grey",fg="white", font=("calibri", 16, "bold"))
                ...
                labelAddress = Label(framebrights, text="Address:", bg="grey",fg="white", font=("calibri", 16, "bold"))
                ...
                labelDisease = Label(framebrights, text="Doctor Name:", bg="grey",fg="white", font=("calibri", 16, "bold"))
                ...
                labelCheckOut = Label(framebrights, text="Check Out:", bg="grey",fg="white", font=("calibri", 16, "bold"))
                ...
                framebottomr = Frame(framebillrecord, relief="solid", bg="grey", borderwidth=2)
                framebottomr.pack(side=TOP, fill=X, pady=16)

                labelPatient_id = Label(framebottomr, text="Sr. No :", bg="grey", fg="white",
                                        font=("calibri", 20, "bold"))
                labelPatient_id.grid(row=0, column=0, sticky="w", padx=60)
                labelPatient_id = Label(framebottomr, text="Particulars :", bg="grey", fg="white",
                                        font=("calibri", 20, "bold"))
                labelPatient_id.grid(row=0, column=1, sticky="w", padx=60)
                labelPatient_id = Label(framebottomr, text="Amount :", bg="grey", fg="white",
                                        font=("calibri", 20, "bold"))
                ...
                framebox = Frame(framebillrecord, bg="grey")
                framebox.pack(side=TOP, fill=BOTH, expand=True, pady="10")

                labelPatient_id = Label(framebox, text="01 :", bg="grey", fg="white",font=("calibri", 17, "bold"))
                labelPatient_id.grid(row=0, column=0, sticky="w", padx=70)
                labelPatient_id = Label(framebox, text="Doctor Charges:", bg="grey",fg="white", font=("calibri", 17, "bold"))
                ...
                labelName = Label(framebox, text="02 :", bg="grey", fg="white",font=("calibri", 17, "bold"))
                labelName.grid(row=1, column=0, sticky="w", padx=70)
                labelName = Label(framebox, text="Lab Test:", bg="grey", fg="white",font=("calibri", 17, "bold"))
                ...
                labelGender = Label(framebox, text="03 :", bg="grey",fg="white", font=("calibri", 17, "bold"))
                labelGender.grid(row=2, column=0, sticky="w", padx=70)
                labelGender = Label(framebox, text="Treatment Charges:", bg="grey", fg="white",font=("calibri", 17, "bold"))
                ...
                labelAge = Label(framebox, text="04 :", bg="grey",fg="white", font=("calibri", 17, "bold"))
                labelAge.grid(row=3, column=0, sticky="w", padx=70)
                labelAge = Label(framebox, text="Procedure Charges:", bg="grey",fg="white", font=("calibri", 17, "bold"))
                ...
                labelPhone = Label(framebox, text="05 :", bg="grey",fg="white", font=("calibri", 17, "bold"))
                labelPhone.grid(row=4, column=0, sticky="w", padx=70)
                labelPhone = Label(framebox, text="Other Charges:", bg="grey", fg="white",font=("calibri", 17, "bold"))
                ...
                labelAddress = Label(framebox, text="06 :", bg="grey",fg="white", font=("calibri", 17, "bold"))
                labelAddress.grid(row=5, column=0, sticky="w", padx=70)
                labelAddress = Label(framebox, text="Miscellaneous Charges:", bg="grey",fg="white",
                                     font=("calibri", 17, "bold"))
                labelAddress.grid(row=5, column=1, sticky="w", padx=45)

                BlrecordMiscellaneouscharges = StringVar()
                Miscellaneouscharges = Entry(framebox, textvariable=BlrecordMiscellaneouscharges,
                                             font=("calibri", 17, "bold"))
                Miscellaneouscharges.grid(row=5, column=2, sticky="w")

                labelAddress = Label(framebox, text="07 :", bg="grey",fg="white", font=("calibri", 17, "bold"))
                labelAddress.grid(row=6, column=0, sticky="w", padx=70)
                labelAddress = Label(framebox, text="Room Charges:", bg="grey",fg="white", font=("calibri", 17, "bold"))
                ...
                labelDisease = Label(framebox, text="", bg="grey",fg="white", font=("calibri", 17, "bold"))
                labelDisease.grid(row=7, column=0, sticky="w", padx=105)
                labelDisease = Label(framebox, text="Total:", fg="red", bg="grey", font=("calibri", 17, "bold"))
        ...
        def Bill():
            ...
            try:
                ...
                pass
            if counterbill == 1:
                framebill = Frame(root, relief="solid", borderwidth=2, bg="grey")
                framebill.pack(side="right", fill=BOTH, expand=True)

                framerecordleft = Frame(framebill, height="60", width="200", relief="solid", borderwidth=2,
                                        bg="grey")
                framerecordleft.pack(side=LEFT, fill=Y)

                framepatientbilltop = Frame(framerecordleft, height="100", relief="solid", borderwidth=2,
                                            bg="white")
                framepatientbilltop.pack(side=TOP, fill=X)

                labelinputpatientname = Label(framepatientbilltop, text="Input Patient's ID", bg="white",fg="black",
                                              font=("calibri", 16, "bold"))
                labelinputpatientname.pack(anchor=CENTER, pady=6)

                framepatientadmittopsearch = Frame(framerecordleft, bg="grey")
                framepatientadmittopsearch.pack(side=TOP)

                searchID = StringVar()
                inputpatientname = Entry(framepatientadmittopsearch, textvariable=searchID, width="15",
                                         font=("calibri", 15, "bold"))
                inputpatientname.pack(pady=10)

                Searchbutton = Button(framepatientadmittopsearch, text="Search",
                                      command=lambda: Billsearch(searchID.get()), bg="white",
                                      font=("calibri", 15, "bold"))
                Searchbutton.pack(side=TOP, pady=8)

                framexbill = Frame(framebill, bg="white", relief="solid", borderwidth=2)
                framexbill.pack(side=TOP, fill=X)

                labelbill = Label(framexbill, text="Billing", bg="white", fg="black", font=("calibri", 20, "bold"))
                labelbill.pack()

                frametbill = Frame(framebill, height="200", bg="grey")
                frametbill.pack(side=TOP, fill=X)

                framebleft = Frame(frametbill, height="200", width="685", bg="grey")
                framebleft.pack(side=LEFT)

                labelPatient_id = Label(framebleft, text="Identity:", bg="grey",fg="white", font=("calibri", 17, "bold"))
                ...
                labelName = Label(framebleft, text="Name:", bg="grey", fg="white",font=("calibri", 17, "bold"))
                ...
                labelGender = Label(framebleft, text="Gender:  ", bg="grey",fg="white", font=("calibri", 17, "bold"))
                labelGender.grid(row=2, column=0)

                i = IntVar()
                r1 = Radiobutton(framebleft, text="male", value='Male', variable=GenderBl, bg="grey",fg="white",
                                 font=("calibri", 16, "bold"))
                r2 = Radiobutton(framebleft, text="female", value='Female', variable=GenderBl, bg="grey",fg="white",
                                 font=("calibri", 16, "bold"))
                r1.grid(row=2, column=1, sticky="w")
                r2.grid(row=2, column=1, sticky="e", padx=0)
                GenderBl.set(3)

                labelAge = Label(framebleft, text="Age:", bg="grey", fg="white",font=("calibri", 17, "bold"))
                ...
                checkin = Label(framebleft, text="Check In:", bg="grey", fg="white",font=("calibri", 17, "bold"))
                ...
                framebright = Frame(frametbill, height="200", bg="grey", width="685")
                framebright.pack(side=RIGHT)

                labelAge = Label(framebright, text="Disease:", bg="grey",fg="white", font=("calibri", 17, "bold"))
                ...
                labelPhone = Label(framebright, text="Phone:", bg="grey", fg="white",font=("calibri", 17, "bold"))
                ...
                labelAddress = Label(framebright, text="Address:", bg="grey",fg="white", font=("calibri", 17, "bold"))
                ...
                labelDisease = Label(framebright, text="Doctor Name:", bg="grey",fg="white", font=("calibri", 17, "bold"))
                ...
                labelCheckOut = Label(framebright, text="Check Out:", bg="grey", fg="white",font=("calibri", 17, "bold"))
                ...
                framebottom = Frame(framebill, relief="solid", bg="grey", borderwidth=2)
                framebottom.pack(side=TOP, fill=X, pady=16)

                labelPatient_id = Label(framebottom, text="Sr. No :", bg="grey", fg="white",
                                        font=("calibri", 20, "bold"))
                labelPatient_id.grid(row=0, column=0, sticky="w", padx=55)
                labelPatient_id = Label(framebottom, text="Particulars :", bg="grey", fg="white",
                                        font=("calibri", 20, "bold"))
                labelPatient_id.grid(row=0, column=1, sticky="w", padx=10)
                labelPatient_id = Label(framebottom, text="Amount :", bg="grey", fg="white",
                                        font=("calibri", 20, "bold"))
                ...
                framebo = Frame(framebill, bg="grey")
                framebo.pack(side=TOP, fill=BOTH, expand=True, pady="10")

                labelPatient_id = Label(framebo, text="01 :", bg="grey",fg="white", font=("calibri", 17, "bold"))
                labelPatient_id.grid(row=0, column=0, sticky="w", padx=75)
                labelPatient_id = Label(framebo, text="Doctor Charges:", bg="grey", fg="white",font=("calibri", 17, "bold"))
                labelPatient_id.grid(row=0, column=1, sticky="w", padx=10)
                ...
                labelName = Label(framebo, text="02 :", bg="grey", fg="white",font=("calibri", 17, "bold"))
                labelName.grid(row=1, column=0, sticky="w", padx=75)
                labelName = Label(framebo, text="Lab Test:", bg="grey", fg="white",font=("calibri", 17, "bold"))
                ...
                labelGender = Label(framebo, text="03 :", bg="grey", fg="white",font=("calibri", 17, "bold"))
                labelGender.grid(row=2, column=0, sticky="w", padx=75)
                labelGender = Label(framebo, text="Treatment Charges:", bg="grey",fg="white", font=("calibri", 17, "bold"))
                ...
                labelAge = Label(framebo, text="04 :", bg="grey",fg="white", font=("calibri", 17, "bold"))
                labelAge.grid(row=3, column=0, sticky="w", padx=75)
                labelAge = Label(framebo, text="Procedure Charges:", bg="grey",fg="white", font=("calibri", 17, "bold"))
                ...
                labelPhone = Label(framebo, text="05 :", bg="grey", fg="white",font=("calibri", 17, "bold"))
                labelPhone.grid(row=4, column=0, sticky="w", padx=75)
                labelPhone = Label(framebo, text="Other Charges:", bg="grey", fg="white",font=("calibri", 17, "bold"))
                ...
                labelAddress = Label(framebo, text="06 :", bg="grey", fg="white",font=("calibri", 17, "bold"))
                labelAddress.grid(row=5, column=0, sticky="w", padx=75)
                labelAddress = Label(framebo, text="Miscellaneous Charges:", bg="grey",fg="white",
                                     font=("calibri", 17, "bold"))
                ...
                labelAddress = Label(framebo, text="07 :", bg="grey", fg="white",font=("calibri", 17, "bold"))
                labelAddress.grid(row=6, column=0, sticky="w", padx=75)
                labelAddress = Label(framebo, text="Room Charges:", bg="grey", fg="white",font=("calibri", 17, "bold"))
                ...
                labelDisease = Label(framebo, text="", bg="grey", fg="white",font=("calibri", 17, "bold"))
                ...
                frameprint = Frame(framebill, bg="grey")
                frameprint.pack(side=RIGHT)

                buttonadd = Button(frameprint, text="SAVE", bg="yellow", command=savebill, bd=3, width=10,
                                   font=("calibri", 15, "bold"))
                buttonadd.pack(side=LEFT)

                buttonPrint = Button(frameprint, text="Print", bg="yellow", command=printbill, bd=3, width=10,
                                     font=("calibri", 15, "bold"))
        ...
        def drecord():
            ...
            if counterdrecord == 1:
                ...
                scroll2.pack(side=RIGHT, fill=Y)
                Staffregistration_tables = ttk.Treeview(framedoctorrecord, columns=(
                "SrID", "SrName", "GenderSr", "SrPosition", "SrDepartment", "SrMailid", "SrSalary", "SrPhone",
                "SrAddress"), xscrollcommand=scroll1.set, yscrollcommand=scroll2.set)
        ...
        def search_dataregistration():
            ...
            cursor.execute("select * from pr where " + str(search_byregistration.get()) + " Like '%" + str(
                search_txtregistration.get()) + "%'")
            rows = cursor.fetchall()
            if len(rows) != 0:
        ...
        def RRecord():
            ...
            if counterrrecord == 1:
                ...
                lbl_search = Label(framerrecordupper, text="Search By", fg="black",
                                   font=("times new roman", 13, "bold"))
                lbl_search.grid(row=0, column=0, pady=7, sticky="w")

                search_byregistration = StringVar()
                combo_search = ttk.Combobox(framerrecordupper, textvariable=search_byregistration, width=7,
                                            font=("times new roman", 13, "bold"), state='readonly')
                combo_search['values'] = ("PrPhone", "PrName", "PrPatientid", "GenderPr", "PrDisease", "PrBloodgroup")
                combo_search.grid(row=0, column=1, pady=10)

                search_txtregistration = StringVar()
                txt_search = Entry(framerrecordupper, textvariable=search_txtregistration,
                                   font=("times new roman", 10, "bold"), bd=5, relief=GROOVE, width=10)
                txt_search.grid(row=0, column=2, padx=5, pady=10, sticky="w")

                searchbtn = Button(framerrecordupper, text="Search", command=search_dataregistration, width=10).grid(
                    row=0, column=3, pady=10)
                showallbtn = Button(framerrecordupper, text="Show All", command=fetchappointment_dataregistration,
                                    width=10).grid(row=0, column=4, pady=10)
                ...
                scroll2.pack(side=RIGHT, fill=Y)
                Patientregistration_table = ttk.Treeview(framerrecorddown, columns=(
                "PrPatientid", "PrName", "GenderPr", "PrAge", "PrPhone", "PrAddress",
                "PrDisease", "PrCheckIn", "PrBloodgroup", "PrDoctorname", "PrRoomNo"), xscrollcommand=scroll1.set,
                                                         yscrollcommand=scroll2.set)
        ...
        def Siupdate():
            connection = sqlite3.connect("SHIVANI.db")
            cursor = connection.cursor()
            query = "update sr set SrName = '{}',GenderSr = '{}',SrPosition = '{}',SrDepartment = '{}',SrMailid = '{}',SrSalary = '{}',SrPhone = '{}',SrAddress = '{}' where SrID = '{}'".format(
                SiName.get(), GenderSi.get(), SiPosition.get(), SiDepartment.get(), SiMailid.get(), SiSalary.get(),
                SiPhone.get(), SiAddress.get(), SiID.get())
            cursor.execute(query)
            connection.commit()
            messagebox.showinfo("Congratulations ", "Data Updated Successfully")
        ...
        def Sradd():
            ...
            if SrName.get() == "" or GenderSr.get() == "" or SrPosition.get() == "" or SrDepartment.get() == "" or SrPhone.get() == "":
                ...
                if "@" and ".com" not in SrMailid.get():
                    ...
                    SrID.set(randomRef)
                    query = "insert into sr values('{}','{}','{}','{}','{}','{}','{}','{}','{}')".format(SrID.get(),
                                                                                                         SrName.get(),
                                                                                                         GenderSr.get(),
                                                                                                         SrPosition.get(),
                                                                                                         SrDepartment.get(),
        ...
        def sinfo():
            ...
            if countersinfo == 1:
                framestaffinformation = Frame(root, bg="grey", )
                framestaffinformation.pack(side="right", fill=BOTH, expand=True)

                framestaffinformationtop = Frame(framestaffinformation, height="200", relief="solid", bg="white",
                                                 borderwidth=2)
                framestaffinformationtop.pack(side=TOP, fill=X)

                labelstaffinformation = Label(framestaffinformationtop, text="Staff Information", bg="white",
                                              font=("calibri", 23, "bold"))
                labelstaffinformation.pack(anchor="center")

                framestaffinformationleft = Frame(framestaffinformation, height="60", width="300", bg="grey",
                                                  relief="solid", borderwidth=2)
                framestaffinformationleft.pack(side=LEFT, fill=Y)

                labelUsername = Label(framestaffinformationleft, text="User ID", bg="grey",fg="white",
                                      font=("calibri", 20, "bold"))
                labelUsername.pack(anchor=CENTER, pady=20)

                searchID = StringVar()
                Username = Entry(framestaffinformationleft, width="20", textvariable=searchID,
                                 font=("calibri", 15, "bold"))
                Username.pack()

                Searchbutton = Button(framestaffinformationleft, text="Search",
                                      command=lambda: Sisearch(searchID.get()), bg="yellow",
                                      font=("calibri", 15, "bold"))
                Searchbutton.pack(side=TOP, pady=8)

                framestaffinformationright = Frame(framestaffinformation, height="60", bg="grey", width="300",
                                                   relief="solid", borderwidth=2)
                framestaffinformationright.pack(side=RIGHT, fill=Y)

                buttonUpdate = Button(framestaffinformationright, text="Update", command=Siupdate, bd=7, bg="yellow",
                                      width=10, font=("calibri", 15, "bold"))
                buttonUpdate.pack(side=TOP, padx="70", pady=100)

                framestaffinformationcenter = Frame(framestaffinformation, height="60", width="130", bg="grey",
                                                    relief="solid", borderwidth=2)
                framestaffinformationcenter.pack(anchor="center", fill=BOTH, expand=True)

                labelid = Label(framestaffinformationcenter, text="ID:                 ", bg="grey",fg="white",
                                font=("calibri", 20, "bold"))
                ...
                labelName = Label(framestaffinformationcenter, text="Name:            ", bg="grey",fg="white",
                                  font=("calibri", 20, "bold"))
                ...
                labelGender = Label(framestaffinformationcenter, text="Gender:          ", bg="grey",fg="white",
                                    font=("calibri", 20, "bold"))
                labelGender.grid(row=2, column=0)

                r1 = Radiobutton(framestaffinformationcenter, text="male", value='Male', bg="grey",fg="white",
                                 variable=GenderSi, font=("calibri", 20, "bold"))
                r1.grid(row=2, column=1, sticky="w")
                r2 = Radiobutton(framestaffinformationcenter, text="female", value='Female', bg="grey",fg="white",
                                 variable=GenderSi, font=("calibri", 20, "bold"))
                r2.grid(row=2, column=1, sticky="e", padx=0)
                GenderSi.set(3)

                labelPosition = Label(framestaffinformationcenter, text="Position:         ", bg="grey",fg="white",
                                      font=("calibri", 20, "bold"))
                ...
                labelDepartment = Label(framestaffinformationcenter, text="Department:         ", bg="grey",fg="white",
                                        font=("calibri", 20, "bold"))
                ...
                labelMailid = Label(framestaffinformationcenter, text="Mail ID:         ", bg="grey",fg="white",
                                    font=("calibri", 20, "bold"))
                ...
                labelSalary = Label(framestaffinformationcenter, text="Salary:            ", bg="grey",fg="white",
                                    font=("calibri", 20, "bold"))
                ...
                labelPhone = Label(framestaffinformationcenter, text="Phone:           ", bg="grey",fg="white",
                                   font=("calibri", 20, "bold"))
                ...
                labelAddress = Label(framestaffinformationcenter, text="Address:         ", bg="grey",fg="white",
                                     font=("calibri", 20, "bold"))
        ...
        def sreg():
            ...
            if countersreg == 1:
                framestaffregistration = Frame(root, bg="grey")
                framestaffregistration.pack(side="right", fill=BOTH, expand=True)

                framestaffregistrationtop = Frame(framestaffregistration, height="200", bg="white", relief="solid",
                                                  borderwidth=2, )
                framestaffregistrationtop.pack(side=TOP, fill=X)

                labelstaffregistration = Label(framestaffregistrationtop, text="Staff Registration", bg="white",
                                               font=("calibri", 23, "bold"))
                labelstaffregistration.pack(anchor="center")

                framestaffregistrationcenter = Frame(framestaffregistration, height="60", width="130", relief="solid",
                                                     bg="grey", borderwidth=2)
                framestaffregistrationcenter.pack(side=LEFT, fill=BOTH, expand=True)

                labelid = Label(framestaffregistrationcenter, text="ID:                 ", bg="grey",fg="white",
                ...
                labelName = Label(framestaffregistrationcenter, text="Name:            ", bg="grey",fg="white",
                                  font=("calibri", 21, "bold"))
                labelName.grid(row=1, column=0, padx=10, pady=15)
                ...
                labelGender = Label(framestaffregistrationcenter, text="Gender:          ", bg="grey",fg="white",
                                    font=("calibri", 21, "bold"))
                labelGender.grid(row=2, column=0)

                i = IntVar()
                r1 = Radiobutton(framestaffregistrationcenter, text="male", value="Male", bg="grey",fg="white",
                                 variable=GenderSr, font=("calibri", 20, "bold"))
                r2 = Radiobutton(framestaffregistrationcenter, text="female", value="Female", variable=GenderSr,
                                 bg="grey", fg="white",font=("calibri", 20, "bold"))
                r1.grid(row=2, column=1, sticky="w")
                r2.grid(row=2, column=1, sticky="e", padx=0)
                GenderSr.set(3)

                labelPosition = Label(framestaffregistrationcenter, text="Position:         ", bg="grey",fg="white",
                                      font=("calibri", 21, "bold"))
                labelPosition.grid(row=3, column=0, padx=10, pady=15)
                ...
                labelDepartment = Label(framestaffregistrationcenter, text="Department:         ", bg="grey",fg="white",
                                        font=("calibri", 21, "bold"))
                labelDepartment.grid(row=4, column=0, padx=10, pady=15)

                SrDepartment = StringVar()
                Department = Entry(framestaffregistrationcenter, textvariable=SrDepartment,
                                   font=("calibri", 20, "bold"))
                Department.grid(row=4, column=1, padx=0)

                labelMailid = Label(framestaffregistrationcenter, text="Mail ID:         ", bg="grey",fg="white",
                                    font=("calibri", 21, "bold"))
                labelMailid.grid(row=5, column=0, padx=10, pady=15)
                ...
                labelSalary = Label(framestaffregistrationcenter, text="Salary:            ", bg="grey",fg="white",
                                    font=("calibri", 21, "bold"))
                labelSalary.grid(row=6, column=0, padx=10, pady=15)
                ...
                labelPhone = Label(framestaffregistrationcenter, text="Phone:           ", bg="grey",fg="white",
                                   font=("calibri", 21, "bold"))
                labelPhone.grid(row=7, column=0, padx=10, pady=15)
                ...
                labelAddress = Label(framestaffregistrationcenter, text="Address:         ", bg="grey",fg="white",
                                     font=("calibri", 21, "bold"))
                labelAddress.grid(row=8, column=0, padx=10, pady=15)
                ...
                framestaffregistrationright = Frame(framestaffregistration, height="60", width="300", bg="grey",
                                                    relief="solid", borderwidth=2)
                framestaffregistrationright.pack(side=RIGHT, fill=Y)

                buttonAdd = Button(framestaffregistrationright, text="ADD", command=Sradd, bd=7, bg="yellow", width=10,
                                   font=("calibri", 15, "bold"))
        ...
        def Aisearch(ID):
            connection = sqlite3.connect("SHIVANI.db")
            cursor = connection.cursor()
            query = "select * from pa where PaPatientid like '{0}' or PaName like '{0}' or PaPhone like '{0}'".format(
                ID)
            cursor.execute(query)
            for row in cursor:
        ...
        def Aiupdate():
            connection = sqlite3.connect("SHIVANI.db")
            cursor = connection.cursor()
            query = "update pa set PaName = '{}',GenderPa = '{}',PaAge = '{}',PaPhone = '{}',PaAddress = '{}',PaDisease = '{}',PaCheckIn = '{}',PaBloodgroup = '{}',PaDoctorname = '{}',PaCheckOut = '{}',PaRoomNo = '{}',PaRoomtype = '{}',PaPrice = '{}' where PaPatientid = '{}'".format(
                AiName.get(), GenderAi.get(), AiAge.get(), AiPhone.get(), AiAddress.get(), AiDisease.get(),
                AiCheckIn.get(), AiBloodgroup.get(), AiDoctorname.get(), AiCheckOut.get(), AiRoomNo.get(),
                AiRoomtype.get(), AiPrice.get(), AiPatientid.get())
            cursor.execute(query)
            connection.commit()
            messagebox.showinfo("Congratulations ", "Data Updated Successfully")
        ...
        def search_dataadmitrecord():
            ...
            cursor.execute("select * from pa where " + str(search_byadmitrecord.get()) + " Like '%" + str(
                search_txtadmitrecord.get()) + "%'")
            rows = cursor.fetchall()
            if len(rows) != 0:
                Patientadmit_table.delete(*Patientadmit_table.get_children())
                for row in rows:
        ...
        def AdmitRecord():
            ...
            if counteradmitrecord == 1:
                ...
                lbl_search = Label(frameadmitrecordupper, text="Search By", fg="black",
                                   font=("times new roman", 13, "bold"))
                lbl_search.grid(row=0, column=0, pady=7, sticky="w")

                search_byadmitrecord = StringVar()
                combo_search = ttk.Combobox(frameadmitrecordupper, textvariable=search_byadmitrecord, width=7,
                                            font=("times new roman", 13, "bold"), state='readonly')
                combo_search['values'] = ("PaPatientid", "PaName", "GenderPa", "PaDisease", "PaPhone", "PaBloodgroup")
                combo_search.grid(row=0, column=1, pady=10)

                search_txtadmitrecord = StringVar()
                txt_search = Entry(frameadmitrecordupper, textvariable=search_txtadmitrecord,
                                   font=("times new roman", 10, "bold"), bd=5, relief=GROOVE, width=10)
                txt_search.grid(row=0, column=2, padx=5, pady=10, sticky="w")

                searchbtn = Button(frameadmitrecordupper, text="Search", command=search_dataadmitrecord, width=10).grid(
                    row=0, column=3, pady=10)
                showallbtn = Button(frameadmitrecordupper, text="Show All", command=fetch_dataadmitrecord,
                                    width=10).grid(row=0, column=4, pady=10)
                ...
                scroll2.pack(side=RIGHT, fill=Y)
                Patientadmit_table = ttk.Treeview(frameadmitrecorddown, columns=(
                "PaPatientid", "PaName", "GenderPa", "PaAge", "PaPhone", "PaAddress", "PaDisease", "PaCheckIn",
                "PaBloodgroup", "PaDoctorname", "PaCheckOut", "PaRoomNo", "PaRoomtype"), xscrollcommand=scroll1.set,
                                                  yscrollcommand=scroll2.set)
        ...
        def ainfo():
            ...
            if counterainfo == 1:
                frameainfo = Frame(root, width=100, height=100, bg="grey")
                frameainfo.pack(side="right", fill=BOTH, expand=TRUE)

                frameainfotop = Frame(frameainfo, height="200", relief="solid", borderwidth=2, bg="white")
                frameainfotop.pack(side=TOP, fill=X)

                labelpatientinfotext = Label(frameainfotop, text="Admit Information", bg="white",
                                             font=("calibri", 23, "bold"))
                labelpatientinfotext.pack(anchor="center")

                frameainfoleft = Frame(frameainfo, height="60", width="300", relief="solid", bg="grey",
                                       borderwidth=2)
                frameainfoleft.pack(side=LEFT, fill=Y)

                labelinputpatientname = Label(frameainfoleft, text="Input Patient's ID", bg="grey",fg="white",
                                              font=("calibri", 17, "bold"))
                labelinputpatientname.pack(anchor=CENTER, pady=20)

                searchID = StringVar()
                inputpatientname = Entry(frameainfoleft, textvariable=searchID, width="20",
                                         font=("calibri", 15, "bold"))
                inputpatientname.pack()

                Search1button = Button(frameainfoleft, text="Search", command=lambda: Aisearch(searchID.get()),
                                       bg="yellow", font=("calibri", 15, "bold"))
                Search1button.pack(side=TOP, pady=8)

                frameainfocenter = Frame(frameainfo, height="60", width="130", relief="solid", bg="grey",
                                         borderwidth=2)
                frameainfocenter.pack(side=LEFT, fill=BOTH, expand=True)

                labelPatientid = Label(frameainfocenter, text="Patient ID:    ", bg="grey",fg="white",
                                       font=("calibri", 20, "bold"))
                ...
                labelPatientid = Label(frameainfocenter, text="", bg="grey", font=("calibri", 20, "bold"))
                labelPatientid.grid(row=0, column=2, sticky="e")

                labelName = Label(frameainfocenter, text="Name:       ", bg="grey",fg="white", font=("calibri", 20, "bold"))
                ...
                labelGender = Label(frameainfocenter, text="Gender:        ", bg="grey",fg="white",
                                    font=("calibri", 20, "bold"))
                labelGender.grid(row=2, column=0)

                i = IntVar()
                r1 = Radiobutton(frameainfocenter, text="male", value=1, variable=GenderAi, bg="grey",fg="white",
                                 font=("calibri", 20, "bold"))
                r2 = Radiobutton(frameainfocenter, text="female", value=2, variable=GenderAi, bg="grey",fg="white",
                                 font=("calibri", 20, "bold"))
                r1.grid(row=2, column=1, sticky="w")
                r2.grid(row=2, column=1, sticky="e", padx=0)
                GenderAi.set(3)

                labelAge = Label(frameainfocenter, text="Age:         ", bg="grey", fg="white",font=("calibri", 20, "bold"))
                ...
                labelPhone = Label(frameainfocenter, text="Phone:     ", bg="grey", fg="white",font=("calibri", 20, "bold"))
                ...
                labelAddress = Label(frameainfocenter, text="Address:  ", bg="grey", fg="white",font=("calibri", 20, "bold"))
                ...
                labelDisease = Label(frameainfocenter, text="Disease:   ", bg="grey",fg="white",
                                     font=("calibri", 20, "bold"))
                ...
                labelCheckIn = Label(frameainfocenter, text="Check In", bg="grey", fg="white",font=("calibri", 20, "bold"))
                ...
                frameainforight = Frame(frameainfo, height="60", width="300", relief="solid", borderwidth=2,
                                        bg="grey")
                frameainforight.pack(side=RIGHT, fill=Y)

                labelBloodgroup = Label(frameainforight, text="Blood Group", bg="grey",fg="white",
                                        font=("calibri", 17, "bold"))
                ...
                labelDoctorname = Label(frameainforight, text="Doctor Name", bg="grey",fg="white",
                                        font=("calibri", 17, "bold"))
                ...
                labelCheckOut = Label(frameainforight, text="Check OUT", bg="grey",fg="white", font=("calibri", 17, "bold"))
                ...
                labelRoomNo = Label(frameainforight, text="Room No.", bg="grey",fg="white", font=("calibri", 17, "bold"))
                ...
                labelRoomType = Label(frameainforight, text="Room Type", bg="grey",fg="white", font=("calibri", 17, "bold"))
                ...
                labelPrice = Label(frameainforight, text="Price.", bg="grey", fg="white",font=("calibri", 17, "bold"))
                ...
                Updatebutton = Button(frameainforight, text="Update", command=Aiupdate, bg="yellow", width=15, bd=7,
                                      font=("calibri", 15, "bold"))
        ...
        def Paadd():
            ...
            if PaPatientid.get() == "" or PaName.get() == "" or GenderPa.get() == "" or PaAge.get() == "" or PaPhone.get() == "" or PaDisease.get() == "" or PaCheckIn.get() == "" or PaCheckOut.get() == "" or PaRoomNo.get() == "" or PaPrice.get() == "":
                ...
                query = "insert into pa values('{}','{}','{}','{}','{}','{}','{}','{}','{}','{}','{}','{}','{}','{}')".format(
                    PaPatientid.get(), PaName.get(), GenderPa.get(), PaAge.get(), PaPhone.get(), PaAddress.get(),
                    PaDisease.get(), PaCheckIn.get(), PaBloodgroup.get(), PaDoctorname.get(), PaCheckOut.get(),
                    PaRoomNo.get(), PaRoomtype.get(), PaPrice.get())
                cursor.execute(query)
                connection.commit()
        ...
        def padmit():
            ...
            if counterpadmit == 1:
                framepatientadmit = Frame(root, width=100, bg="grey", height=100)
                framepatientadmit.pack(side="right", fill=BOTH, expand=TRUE)

                frameainfoleft = Frame(framepatientadmit, height="60", width="200", relief="solid", borderwidth=2,
                                       bg="grey")
                frameainfoleft.pack(side=LEFT, fill=Y)

                framepatientadmittop = Frame(frameainfoleft, height="200", relief="solid", borderwidth=2,
                                             bg="white")
                framepatientadmittop.pack(side=TOP, fill=X)

                labelinputpatientname = Label(framepatientadmittop, text="Input Patient's ID", bg="white",
                                              font=("calibri", 17, "bold"))
                labelinputpatientname.pack(anchor=CENTER, pady=10)

                framepatientadmittopsearch = Frame(frameainfoleft, bg="grey")
                framepatientadmittopsearch.pack(side=TOP)
                ...
                Searchbutton = Button(framepatientadmittopsearch, text="Search",
                                      command=lambda: Adsearch(searchID.get()), bg="yellow",
                                      font=("calibri", 15, "bold"))
                Searchbutton.pack(side=TOP, pady=8)

                framepatientadmittop = Frame(framepatientadmit, height="200", relief="solid", borderwidth=2,
                                             bg="white")
                framepatientadmittop.pack(side=TOP, fill=X)

                labelpatientrinfotext = Label(framepatientadmittop, text="Patient Admit", bg="white",
                                              font=("calibri", 23, "bold"))
                labelpatientrinfotext.pack(anchor="center")

                framepatientadmitcenter = Frame(framepatientadmit, height="60", width="130", bg="grey",
                                                relief="solid", borderwidth=2)
                framepatientadmitcenter.pack(side=LEFT, fill=BOTH, expand=True)

                labelPatientid = Label(framepatientadmitcenter, text="Patient ID:    ", bg="grey",fg="white",
                                       font=("calibri", 20, "bold"))
                ...
                labelName = Label(framepatientadmitcenter, text="Name:       ", bg="grey",fg="white",
                                  font=("calibri", 20, "bold"))
                ...
                labelGender = Label(framepatientadmitcenter, text="Gender:        ", bg="grey",fg="white",
                                    font=("calibri", 20, "bold"))
                labelGender.grid(row=2, column=0)

                i = IntVar()
                r1 = Radiobutton(framepatientadmitcenter, text="male", value=1, variable=GenderPa, bg="grey",fg="white",
                                 font=("calibri", 20, "bold"))
                r2 = Radiobutton(framepatientadmitcenter, text="female", value=2, variable=GenderPa, bg="grey",fg="white",
                                 font=("calibri", 20, "bold"))
                r1.grid(row=2, column=1, sticky="w")
                r2.grid(row=2, column=1, sticky="e", padx=0)
                GenderPa.set(3)

                labelAge = Label(framepatientadmitcenter, text="Age:         ", bg="grey",fg="white",
                                 font=("calibri", 20, "bold"))
                ...
                labelPhone = Label(framepatientadmitcenter, text="Phone:     ", bg="grey",fg="white",
                                   font=("calibri", 20, "bold"))
                ...
                labelAddress = Label(framepatientadmitcenter, text="Address:  ", bg="grey",fg="white",
                                     font=("calibri", 20, "bold"))
                ...
                labelDisease = Label(framepatientadmitcenter, text="Disease:   ", bg="grey",fg="white",
                                     font=("calibri", 20, "bold"))
                ...
                labelCheckIn = Label(framepatientadmitcenter, text="Check In", bg="grey",fg="white",
                                     font=("calibri", 20, "bold"))
                ...
                framepatientadmitright = Frame(framepatientadmit, height="60", width="300", relief="solid",
                                               bg="grey", borderwidth=2)
                framepatientadmitright.pack(side=RIGHT, fill=Y)

                labelBloodgroup = Label(framepatientadmitright, text="Blood group", bg="grey",fg="white",
                                        font=("calibri", 17, "bold"))
                labelBloodgroup.pack(padx=80, pady=10)

                PaBloodgroup = StringVar()
                Bloodgroup = Entry(framepatientadmitright, textvariable=PaBloodgroup, width="20",
                                   font=("calibri", 17, "bold"))
                Bloodgroup.pack()

                labelDoctorname = Label(framepatientadmitright, text="Doctor Name", bg="grey",fg="white",
                                        font=("calibri", 17, "bold"))
                labelDoctorname.pack(padx=80, pady=10)

                PaDoctorname = StringVar()
                Doctorname = Entry(framepatientadmitright, textvariable=PaDoctorname, width="20",
                                   font=("calibri", 17, "bold"))
                Doctorname.pack()

                labelCheckOut = Label(framepatientadmitright, text="Check OUT", bg="grey",fg="white",
                                      font=("calibri", 17, "bold"))
                labelCheckOut.pack(pady=10)

                PaCheckOut = StringVar()
                CheckOut = Entry(framepatientadmitright, textvariable=PaCheckOut, width="20",
                                 font=("calibri", 17, "bold"))
                CheckOut.pack()

                labelRoomNo = Label(framepatientadmitright, text="Room No.", bg="grey",fg="white",
                                    font=("calibri", 17, "bold"))
                labelRoomNo.pack(pady=10)
                ...
                labelRoomType = Label(framepatientadmitright, text="Room Type", bg="grey",fg="white",
                                      font=("calibri", 17, "bold"))
                labelRoomType.pack(pady=10)
                ...
                labelPrice = Label(framepatientadmitright, text="Price.", bg="grey", fg="white",font=("calibri", 17, "bold"))
                labelPrice.pack(pady=10)
                ...
                ADDAbutton = Button(framepatientadmitright, text="ADD", command=Paadd, bg="yellow", width=15, bd=7,
                                    font=("calibri", 15, "bold"))
        ...
        def Riupdate():
            connection = sqlite3.connect("SHIVANI.db")
            cursor = connection.cursor()
            query = "update pr set PrName = '{}',GenderPr = '{}',PrAge = '{}',PrPhone = '{}',PrAddress = '{}',PrDisease = '{}',PrCheckIn = '{}',PrBloodgroup = '{}',PrDoctorname = '{}',PrRoomNo = '{}' where PrPatientid = '{}'".format(
                RiName.get(), GenderRi.get(), RiAge.get(), RiPhone.get(), RiAddress.get(), RiDisease.get(),
                RiCheckIn.get(), RiBloodgroup.get(), RiDoctorname.get(), RiRoomNo.get(), RiPatientid.get())
            cursor.execute(query)
            connection.commit()
            messagebox.showinfo("Congratulations ", "Data Updated Successfully")
        ...
        def Pradd():
            ...
            if PrName.get() == "" or GenderPr.get() == "" or PrAge.get() == "" or PrPhone.get() == "" or PrAddress.get() == "" or PrDisease.get() == "":
                ...
                query = "insert into pr values('{}','{}','{}','{}','{}','{}','{}','{}','{}','{}','{}')".format(
                    PrPatientid.get(), PrName.get(), GenderPr.get(), int(PrAge.get()), PrPhone.get(), PrAddress.get(),
                    PrDisease.get(), PrCheckIn.get(), PrBloodgroup.get(), PrDoctorname.get(), PrRoomNo.get())
                cursor.execute(query)
                connection.commit()
        ...
        def rinfo():
            ...
            if counterrinfo == 1:
                ...
                framerinfotop = Frame(framerinfo, height="200", relief="solid", borderwidth=2, bg="white")
                framerinfotop.pack(side=TOP, fill=X)

                labelpatientrinfotext = Label(framerinfotop, text="Registration Information", bg="white",
                                              font=("calibri", 23, "bold"))
                labelpatientrinfotext.pack(anchor="center")

                framerinfoleft = Frame(framerinfo, height="60", width="300", relief="solid", borderwidth=2,
                                       bg="grey")
                framerinfoleft.pack(side=LEFT, fill=Y)

                labelinputpatientname = Label(framerinfoleft, text="Input Patient's ID", bg="grey",
                                              font=("calibri", 17, "bold"))
                labelinputpatientname.pack(anchor=CENTER, pady=20)

                searchID = StringVar()
                inputpatientname = Entry(framerinfoleft, textvariable=searchID, width="20",
                                         font=("calibri", 15, "bold"))
                inputpatientname.pack()

                Searchbutton = Button(framerinfoleft, text="Search", command=lambda: Risearch(searchID.get()),
                                      bg="yellow", font=("calibri", 15, "bold"))
                Searchbutton.pack(side=TOP, pady=8)

                framerinfocenter = Frame(framerinfo, height="60", width="130", bg="grey", relief="solid",
                                         borderwidth=2)
                framerinfocenter.pack(side=LEFT, fill=BOTH, expand=True)

                labelPatientid = Label(framerinfocenter, text="Patient ID:    ", bg="grey",fg="white",
                                       font=("calibri", 20, "bold"))
                labelPatientid.grid(row=0, column=0, pady=10, sticky="w")
                ...
                labelPatientid = Label(framerinfocenter, text="", bg="grey",fg="white", font=("calibri", 20, "bold"))
                labelPatientid.grid(row=0, column=2, sticky="e")

                labelName = Label(framerinfocenter, text="Name:       ", bg="grey",fg="white", font=("calibri", 20, "bold"))
                labelName.grid(row=1, column=0, padx=0, pady=17, sticky="w")
                ...
                labelGender = Label(framerinfocenter, text="Gender:        ", bg="grey",fg="white",
                                    font=("calibri", 20, "bold"))
                labelGender.grid(row=2, column=0)

                r1 = Radiobutton(framerinfocenter, text="male", value='Male', variable=GenderRi, bg="grey",fg="white",
                                 font=("calibri", 20, "bold"))
                r1.grid(row=2, column=1, sticky="w")
                r2 = Radiobutton(framerinfocenter, text="female", value='Female', variable=GenderRi, bg="grey",fg="white",
                                 font=("calibri", 20, "bold"))
                r2.grid(row=2, column=1, sticky="e", padx=0)
                GenderRi.set(3)

                labelAge = Label(framerinfocenter, text="Age:         ", bg="grey", fg="white",font=("calibri", 20, "bold"))
                labelAge.grid(row=3, column=0, sticky="w", padx=0, pady=17)
                ...
                labelPhone = Label(framerinfocenter, text="Phone:     ", bg="grey",fg="white", font=("calibri", 20, "bold"))
                labelPhone.grid(row=4, column=0, sticky="w", padx=0, pady=17)
                ...
                labelAddress = Label(framerinfocenter, text="Address:  ", bg="grey",fg="white", font=("calibri", 20, "bold"))
                labelAddress.grid(row=5, column=0, sticky="w", padx=0, pady=17)
                ...
                labelDisease = Label(framerinfocenter, text="Disease:   ", bg="grey",fg="white",
                                     font=("calibri", 20, "bold"))
                labelDisease.grid(row=6, column=0, sticky="w", padx=0, pady=17)
                ...
                labelCheckIn = Label(framerinfocenter, text="Check In", bg="grey",fg="white", font=("calibri", 20, "bold"))
                labelCheckIn.grid(row=7, column=0, sticky="w", padx=0, pady=17)
                ...
                framerinforight = Frame(framerinfo, height="60", width="300", bg="grey", fg="white",relief="solid",
                                        borderwidth=2)
                framerinforight.pack(side=RIGHT, fill=Y)

                labelDoctorname = Label(framerinforight, text="Blood Group", bg="grey",fg="white",
                                        font=("calibri", 17, "bold"))
                labelDoctorname.pack(padx=80, pady=10)
                ...
                labelDoctorname = Label(framerinforight, text="Doctor Name", bg="grey",fg="white",
                                        font=("calibri", 17, "bold"))
                labelDoctorname.pack(padx=80, pady=10)
                ...
                labelDoctorname = Label(framerinforight, text="Room No", bg="grey", fg="white",font=("calibri", 17, "bold"))
                labelDoctorname.pack(padx=80, pady=10)
                ...
                Updatebutton = Button(framerinforight, text="Update", command=Riupdate, bg="yellow", bd=7,
                                      font=("calibri", 15, "bold"))
                Updatebutton.pack(side=RIGHT, padx=35, pady=100)

                Updatebutton = Button(framerinforight, text="Receipt", command=Rireceipt, bg="yellow", bd=7,
                                      font=("calibri", 15, "bold"))
                Updatebutton.pack(side=RIGHT, padx=35, pady=100)
        ...
        def Receipt():
            ...
            try:
                ...
                pass
            if counterreceipt == 1:
                framereceiptimage = Frame(root, bg="grey")
                ...
                #label = Label(framereceiptimage, text="NH-62, Pali Road,Kudi Haud, Jodhpur Rajasthan",
                              #font=("calibri", 15, "bold"), bg="grey")
                ...
                framereceipt = Frame(root, bg="grey")
                framereceipt.pack(side="top", fill=BOTH, expand=True)

                framereceipttop = Frame(framereceipt, bg="grey", height=20)
                framereceipttop.pack(side=TOP, fill=X)

                labelreceipt = Label(framereceipttop, text="Receipt :-", bg="grey", fg="black",
                                     font=('arial', 20, 'bold'))
                ...
                labelreceipt = Label(framereceipt, text="Medicine: - ", fg="black", bg="grey",
                                     font=('arial', 20, 'bold'))
                labelreceipt.pack(anchor="nw", padx=150, pady=10)

                buttonprint = Button(framereceipt, text="Print", width=7, fg="red", font=('arial', 20, 'bold'),
                                     command=iPrint)
        ...
        def Rireceipt():
            ...
            if counterrireceipt == 1:
                framerireceiptimage = Frame(root, bg="grey")
                ...
                label = Label(framerireceiptimage, text="NH-62, Pali Road,Kudi Haud, Jodhpur Rajasthan",
                              font=("calibri", 15, "bold"), bg="grey")
                label.grid(row=0, column=1, padx=200)
                ...
                framerireceipt = Frame(root, bg="grey")
                framerireceipt.pack(side="top", fill=BOTH, expand=True)

                framerireceipttop = Frame(framerireceipt, bg="grey", height=20)
                framerireceipttop.pack(side=TOP, fill=X)

                labelreceipt = Label(framerireceipttop, text="Receipt :-", bg="grey", fg="black",
                                     font=('arial', 20, 'bold'))
                labelreceipt.pack(side=LEFT, padx=150, pady=10)
                ...
                labelreceipt = Label(framerireceipt, text="Medicine: - ", fg="black", bg="grey",
                                     font=('arial', 20, 'bold'))
                labelreceipt.pack(anchor="nw", padx=150, pady=10)

                buttonprint = Button(framerireceipt, text="Print", width=7, fg="red", font=('arial', 20, 'bold'),
                                     command=iPrint)
                buttonprint.pack(side=BOTTOM)
        ...
        def printbill():
            ...
            if counterprintbill == 1:
                frameprintbill = Frame(root, bg="grey")
                frameprintbill.pack(side="top", fill=BOTH, expand=True)

                txtReceiptbill = Text(frameprintbill, width=100, height=17, bg='white', bd=4,
                                      font=('arial', 12, 'bold'))
                txtReceiptbill.pack(anchor="nw", padx=100)

                txtReceiptbill.insert(END,
                                      'Patient id:-\t\t' + BlIdentity.get() + '\t\t' + 'Name:-\t\t\t' + BlName.get() + '\n')
                txtReceiptbill.insert(END,
                                      'Gender:-\t\t' + GenderBl.get() + '\t\t' + 'Age:-\t\t\t' + BlAge.get() + '\n')
                txtReceiptbill.insert(END,
                                      'CheckIn:-\t\t' + Blcheckin.get() + '\t\t' + 'CheckOut:-\t\t\t' + Blcheckout.get() + '\n')
                txtReceiptbill.insert(END,
                                      'Disease:-\t\t' + BlDisease.get() + '\t\t' + 'Phone No:-\t\t\t' + BlPhone.get() + '\n')
                txtReceiptbill.insert(END,
                                      'Address:-\t\t' + BlAddress.get() + '\t\t' + 'Doctor name:-\t\t\t' + BlDoctorname.get() + '\n' + '\n')
                ...
                buttonbill = Button(frameprintbill, text="Print", font=('arial', 12, 'bold'), fg="red",
                                    command=billPrint)
        ...
        def preg():
            ...
            if counterpreg == 1:
                framepatientregistration = Frame(root, width=100, height=100, bg="grey")
                framepatientregistration.pack(side="right", fill=BOTH, expand=TRUE)

                framepatientregistrationtop = Frame(framepatientregistration, height="200", relief="solid",
                                                    borderwidth=2, bg="white")
                framepatientregistrationtop.pack(side=TOP, fill=X)

                labelpatientregistrationtext = Label(framepatientregistrationtop, text="Patient Registration",
                                                     bg="white", font=("calibri", 23, "bold"))
                labelpatientregistrationtext.pack(anchor="center")

                framepatientregistrationcenter = Frame(framepatientregistration, bg="grey", height="60",
                                                       width="130", relief="solid", borderwidth=2)
                framepatientregistrationcenter.pack(side=LEFT, fill=BOTH, expand=True)

                labelPatientid = Label(framepatientregistrationcenter, bg="grey", text="Patient ID:    ",fg="white",
                                       font=("calibri", 20, "bold"))
                labelPatientid.grid(row=0, column=0, padx=90, pady=10, sticky="w")

                PrPatientid = StringVar()
                Patientid = Entry(framepatientregistrationcenter, textvariable=PrPatientid,
                                  font=("calibri", 20, "bold"))
                Patientid.grid(row=0, column=1, padx=1, pady=17, sticky="w")

                labelName = Label(framepatientregistrationcenter, text="Name:       ", bg="grey",fg="white",
                                  font=("calibri", 20, "bold"))
                ...
                labelGender = Label(framepatientregistrationcenter, text="Gender:    ", bg="grey",fg="white",
                                    font=("calibri", 20, "bold"))
                labelGender.grid(row=2, column=0, sticky="w", padx=90)

                i = IntVar()
                r1 = Radiobutton(framepatientregistrationcenter, text="male", value="Male", bg="grey",fg="white",
                                 variable=GenderPr, font=("calibri", 20, "bold"))
                r2 = Radiobutton(framepatientregistrationcenter, text="female", value="Female", bg="grey",fg="white",
                                 variable=GenderPr, font=("calibri", 20, "bold"))
                r1.grid(row=2, column=1, sticky="w")
                r2.grid(row=2, column=1, sticky="e", padx=0)
                GenderPr.set(3)

                labelAge = Label(framepatientregistrationcenter, text="Age:         ", bg="grey",fg="white",
                                 font=("calibri", 20, "bold"))
                ...
                labelPhone = Label(framepatientregistrationcenter, text="Phone:     ", bg="grey",fg="white",
                                   font=("calibri", 20, "bold"))
                ...
                labelAddress = Label(framepatientregistrationcenter, text="Address:  ", bg="grey",fg="white",
                                     font=("calibri", 20, "bold"))
                ...
                labelDisease = Label(framepatientregistrationcenter, text="Disease:   ", bg="grey",fg="white",
                                     font=("calibri", 20, "bold"))
                ...
                labelCheckIn = Label(framepatientregistrationcenter, text="Check IN", bg="grey",fg="white",
                                     font=("calibri", 20, "bold"))
                ...
                framepatientregistrationright = Frame(framepatientregistration, height="60", width="300",
                                                      bg="grey", relief="solid", borderwidth=2)
                framepatientregistrationright.pack(side=RIGHT, fill=Y)

                labelBloodgroup = Label(framepatientregistrationright, text="Blood Group", bg="grey",fg="white",
                                        font=("calibri", 17, "bold"))
                labelBloodgroup.pack(pady=10)

                PrBloodgroup = StringVar()
                Bloodgroup = Entry(framepatientregistrationright, textvariable=PrBloodgroup, width="20",
                                   font=("calibri", 17, "bold"))
                Bloodgroup.pack()

                labelDoctorname = Label(framepatientregistrationright, text="Doctor Name", bg="grey",fg="white",
                                        font=("calibri", 17, "bold"))
                labelDoctorname.pack(pady=10)

                list = ["Rajesh[Dentist]", "Vishal[Cardiologist]", "Gourav[Pediatricians]", "Gourav[Physician]",
                        "Vijay[Therapist]", "Karan[Diagnostic]"]
                ...
                labelRoomNo = Label(framepatientregistrationright, text="Room No.", bg="grey",fg="white",
                                    font=("calibri", 17, "bold"))
                labelRoomNo.pack(pady=10)

                PrRoomNo = StringVar()
                RoomNo = Entry(framepatientregistrationright, textvariable=PrRoomNo, width="20",
                               font=("calibri", 17, "bold"))
                RoomNo.pack(padx=5)

                ADDAbutton = Button(framepatientregistrationright, text="ADD", command=Pradd, bg="yellow", width=10,
                                    bd=7, font=("calibri", 15, "bold"))
                ADDAbutton.pack(side=LEFT, padx=5)

                buttonprintreg = Button(framepatientregistrationright, text="Receipt", command=Receipt, bg="yellow",
                                        width=10, bd=7, font=("calibri", 15, "bold"))
                buttonprintreg.pack(side=LEFT, padx=5)

        framemenuroot = Frame(root, height="190", bg="grey")
        framemenuroot.pack(side=TOP, fill=X)
        framehospitalname = Label(framemenuroot, text="                                  SCS HOSPITAL",
                                  bg="grey", fg="white", width="35", font=("calibri", 27, "bold"))
        ...
        framemenubackground = Frame(root, bg="grey", height="60", width="50")
        framemenubackground.pack(side=LEFT, fill=Y)
        framemenuheading = Label(framemenubackground, text="Menu", fg="black", bg="white", width="25",
                                 font=("calibri", 18, "bold"))
        framemenuheading.pack(anchor="nw")

        #homeicon = PhotoImage(file=r"home.png")
        #Buttonpatienthome = Button(framemenubackground, text="Home            ", compound=LEFT, image=homeicon,
                                   #command=home, bd=5, bg="white", font=("calibri", 14, "bold"))
        #Buttonpatienthome.pack(side="top", fill=X)

        patientregistrationicon = PhotoImage(file=r"patientregistration.png")
        Buttonpatientregistration = Button(framemenubackground, text="Patient Registration         ", compound=LEFT,
                                           image=patientregistrationicon, command=preg, bd=5, bg="white",
                                           font=("calibri", 14, "bold"))
        Buttonpatientregistration.pack(side="top", fill=X)

        patientadmiticon = PhotoImage(file=r"patientadmit.png")
        Buttonpatientinformation = Button(framemenubackground, text="Patient Admit                     ", compound=LEFT,
                                          image=patientadmiticon, command=padmit, bd=5, bg="white",
                                          font=("calibri", 14, "bold"))
        Buttonpatientinformation.pack(side="top", fill=X)

        registrationinformationicon = PhotoImage(file=r"patientinformation.png")
        Buttonregistrationinfo = Button(framemenubackground, text="Registration Information", compound=LEFT,
                                        image=registrationinformationicon, command=rinfo, bd=5, bg="white",
                                        font=("calibri", 14, "bold"))
        Buttonregistrationinfo.pack(side="top", fill=X)

        admitinformationicon = PhotoImage(file=r"patientadmit.png")
        Buttonadmitinformation = Button(framemenubackground, text="Admit Information            ", compound=LEFT,
                                        image=admitinformationicon, command=ainfo, bd=5, bg="white",
                                        font=("calibri", 14, "bold"))
        Buttonadmitinformation.pack(side="top", fill=X)

        doctorregistrationicon = PhotoImage(file=r"doctorregistration.png")
        Buttonstaffregistration = Button(framemenubackground, text="Staff Registration              ", compound=LEFT,
                                         image=doctorregistrationicon, command=sreg, bd=5, bg="white",
                                         font=("calibri", 14, "bold"))
        Buttonstaffregistration.pack(side="top", fill=X)

        doctorinformationicon = PhotoImage(file=r"doctorinformation.png")
        Buttonstaffinformation = Button(framemenubackground, text="Staff Information               ", compound=LEFT,
                                        image=doctorinformationicon, command=sinfo, bd=5, bg="white",
                                        font=("calibri", 14, "bold"))
        Buttonstaffinformation.pack(side="top", fill=X)

        registrationrecordicon = PhotoImage(file=r"registrationrecord.png")
        ButtonRegistrationRecord = Button(framemenubackground, text="Registration Record         ", compound=LEFT,
                                          image=registrationrecordicon, command=RRecord, bd=5, bg="white",
                                          font=("calibri", 14, "bold"))
        ButtonRegistrationRecord.pack(side="top", fill=X)

        admitrecordicon = PhotoImage(file=r"admitrecord.png")
        ButtonAdmitRecord = Button(framemenubackground, text="Admit Record                    ", compound=LEFT,
                                   image=admitrecordicon, command=AdmitRecord, bd=5, bg="white",
                                   font=("calibri", 14, "bold"))
        ButtonAdmitRecord.pack(side="top", fill=X)

        doctorrecordicon = PhotoImage(file=r"doctorrecord.png")
        ButtonStaffRecord = Button(framemenubackground, text="Staff Record                        ", compound=LEFT,
                                   image=doctorrecordicon, command=drecord, bd=5, bg="white",
                                   font=("calibri", 14, "bold"))
        ButtonStaffRecord.pack(side="top", fill=X)

        generatebillicon = PhotoImage(file=r"generatebill.png")
        ButtonBill = Button(framemenubackground, text="Generate Bill                       ", compound=LEFT,
                            image=generatebillicon, command=Bill, bd=5, bg="white", font=("calibri", 14, "bold"))
        ButtonBill.pack(side="top", fill=X)

        billrecordicon = PhotoImage(file=r"recordbill.png")
        ButtonBill = Button(framemenubackground, text="Bill Record                             ", compound=LEFT,
                            image=billrecordicon, command=billrecord, bd=5, bg="white",
                            font=("calibri", 14, "bold"))
        ButtonBill.pack(side="top", fill=X)

        appointmenticon = PhotoImage(file=r"appointment.png")
        Buttonappointment = Button(framemenubackground, text="Appointment                       ", compound=LEFT,
                                   image=appointmenticon, bd=5, bg="white", font=("calibri", 14, "bold"),
                                   command=appointment)
...
userright = Frame(frame1, height="400", width="400", bg="grey", relief="solid", borderwidth=2)
userright.pack(side=RIGHT, padx="200")

usernullframe = Frame(userright, height="50", bg="grey")
usernullframe.pack(side=TOP)

image1 = PhotoImage(file="ee.png")
label = Label(userright, image=image1, width=150, height=150, bg="grey")
label.pack(side=LEFT, padx=10)

usernameicon = PhotoImage(file=r"username.png")
labelUsername = Label(userright, text="Username:", compound=LEFT, image=usernameicon, font=("calibri", 18, "bold"),
                      bg="grey")
...
labelPassword = Label(userright, text="Password:", compound=LEFT, image=passwordicon, font=("calibri", 18, "bold"),
                      bg="grey")
...
buttonok = Button(userright, text="LOGIN", bg="orange", height=2, width=6, bd=3, font=("calibri", 12, "bold"),
                  command=login)
buttonok.pack(side=LEFT, padx="10", pady="10")

buttonexit = Button(userright, text="EXIT", bg="orange", height=2, width=6, bd=3, font=("calibri", 12, "bold"),
                    command=exit)
...
try:
    ...
    cursor = connection.cursor()
    cursor.execute(
        "CREATE TABLE IF NOT EXISTS ap (patientnoappointment TEXT NOT NULL  PRIMARY KEY, patientnameappointment TEXT, genderappointment TEXT, ageappointment TEXT, phonenoappointment TEXT, addressappointment TEXT, appointmenttime TEXT)")
    cursor.execute("SELECT * FROM ap ORDER BY `patientnoappointment` ASC")
    fetch = cursor.fetchall()
    for data in fetch:
        appointment_table.insert('', 'end', values=(data))
